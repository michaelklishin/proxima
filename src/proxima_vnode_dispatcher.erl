...
dispatch(Req) ->
  ...
  case riak_core_vnode_master:sync_spawn_command(Index, {Method, Resource, Req}, proxima_vnode_master) of
    {ok, R} -> R;
  ...
  Read = fun(F) ->
    receive
      {browser, Data} ->
        ...
        riak_core_vnode_master:command(Index, {websocket, string:tokens(Path, "/"), Headers, Data, Ws}, proxima_vnode_master),
        F(F);
      _ ->
        F(F)
    end
